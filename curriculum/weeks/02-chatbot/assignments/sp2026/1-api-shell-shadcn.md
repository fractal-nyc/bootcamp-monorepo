...
- Use `curl` to make a request to the Anthropic API based on these docs: https://platform.claude.com/docs/en/get-started.
  ...
  - Notice the `-d` param to `curl`. This turns it into a `POST` request.
...
- This is going to become tedious, but fortunately Anthropic provides a [NodeJS SDK](https://platform.claude.com/docs/en/api/client-sdks#type-script) for calling their API.
