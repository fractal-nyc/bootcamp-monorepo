...
const ATTENDANCE_CHANNEL_ID = "1418329701658792046";
const CURRENT_COHORT_ROLE = "Fa2025";
...
// ===== MESSAGE TEMPLATES =====
const EOD_STATUS_TEMPLATE = `Reminder @${CURRENT_COHORT_ROLE}, 📝 Please submit your EOD status report
...
const DAILY_READINGS_TEMPLATE = `@${CURRENT_COHORT_ROLE}, this is your daily thread to discuss today's readings.
...
const DAILY_PRACTICE_TEMPLATE = `@${CURRENT_COHORT_ROLE}, this is your daily thread to discuss today's practice problems.
...
const CRON_JOBS: CronJobConfig[] = [
  ...
  },
  {
    ...
    message: {
      content:
        "🌙 This is your midnight cron test message, the crons should work tomorrow!",
  ...
  },
  {
    ...
    message: {
      ...
      mentions: [CURRENT_COHORT_ROLE], // Uses predefined role name
    },
  },
  {
    name: "daily_reading",
  ...
  {
    name: "attendance_reminder_fa_2025",
    schedule: "0 0 9 * * 1-6", // Monday through Saturday at 9:00 AM
...
export function startCronJobs(client: Client): void {
  CRON_JOBS.forEach((jobConfig: CronJobConfig) => {
    if (jobConfig.enabled) {
      ...
      console.log(
        `✅ Started cron job: ${jobConfig.name} (${jobConfig.schedule}) with ${SEVALLA_THRESHOLD_MS}ms threshold`
      );
    } else {
...
export async function sendScheduledMessage(
  ...
  try {
    ...
    if (!channel || !channel.isTextBased()) {
      console.error(
        `❌ Channel not found or not text - based: ${job.channelId} for job: ${job.name} `
      );
      return;
    }
    ...
    console.error(
      `❌ Error sending scheduled message for ${job.name}: `,
      error
    );
  }
}
